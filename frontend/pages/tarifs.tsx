      ...
      <section className="pt-44 pb-32">
        <div className="container mx-auto px-6">
          <div className="text-center mb-24 max-w-3xl mx-auto">
            ...
            <h1 className="text-6xl md:text-7xl font-serif text-[#1a1a1a] mb-8">Consultation Naturopathe <span className="italic text-gradient">Prix</span></h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
            {tarifs.map((t, idx) => (
              <div key={idx} className={`relative p-12 rounded-[3rem] transition-all duration-500 hover:shadow-2xl animate-reveal-up ${t.highlight ? 'bg-[#3fad28] text-white scale-105 z-10' : 'bg-white text-[#1a1a1a]'}`} style={{ animationDelay: `${idx * 0.1}s` }}>
                {t.highlight && <span className="absolute top-8 right-12 text-[8px] font-bold uppercase tracking-widest bg-[#e68d15] px-3 py-1 rounded-full text-white">Populaire</span>}
                ...
                <ul className="space-y-4 mb-12">
                  {t.features.map((f, fIdx) => (
                    <li key={fIdx} className="flex items-center gap-3 text-sm font-light">
                      <Check className={`w-4 h-4 ${t.highlight ? 'text-[#e68d15]' : 'text-[#3fad28]'}`} />
                      {f}
                    </li>
                  ))}
                </ul>
                
                <Link href="/" className={`block w-full py-5 rounded-full text-center text-xs font-bold uppercase tracking-widest transition-all ${t.highlight ? 'bg-[#e68d15] text-white hover:bg-white hover:text-[#3fad28]' : 'bg-[#3fad28] text-white hover:bg-[#e68d15]'}`}>
                  Réserver maintenant
                </Link>
              </div>
            ))}
          ...
          <div className="bg-white rounded-[4rem] p-12 md:p-24 shadow-sm border border-[#3fad28]/5 max-w-5xl mx-auto">
            <h2 className="text-4xl font-serif text-[#1a1a1a] mb-12 text-center">Questions Fréquentes sur les <span className="italic text-gradient">Tarifs</span></h2>
            <div className="grid md:grid-cols-2 gap-12">
              <div>
                <h4 className="text-lg font-serif mb-4 text-[#3fad28]">Combien coûte une consultation naturopathe ?</h4>
              ...
              <div>
                <h4 className="text-lg font-serif mb-4 text-[#3fad28]">Quel est le prix du diagnostic capillaire ?</h4>
              ...
              <div>
                <h4 className="text-lg font-serif mb-4 text-[#3fad28]">Puis-je payer en plusieurs fois ?</h4>
              ...
              <div>
                <h4 className="text-lg font-serif mb-4 text-[#3fad28]">Modes de paiement acceptés</h4>
